...
import { Menu, X, Home, TrendingUp, BookOpen, Target, Calendar, MessageSquare, Settings, Bell, Search } from 'lucide-react';
...
const navigationItems = [
  ...
  { id: 'progress', label: 'Napredek', icon: TrendingUp },
  { id: 'assignments', label: 'Naloge', icon: Target },
...
export function ParentMobileNavigation({
  activeSection,
  onSectionChange,
  session,
  unreadNotifications = 0
}: ParentMobileNavigationProps) {
      ...
      <div className="lg:hidden fixed top-0 left-0 right-0 z-50 bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60 border-b">
        <div className="flex items-center justify-between px-4 py-3">
          <div className="flex items-center space-x-2">
              ...
              <SheetContent side="left" className="w-80">
                <div className="flex flex-col h-full">
                  ...
                  <div className="flex items-center space-x-3 p-4 border-b">
                    ...
                    <div>
                      <p className="font-medium text-sm">{session?.user?.name || 'Starš'}</p>
            ...
            <h1 className="text-lg font-semibold">Staršev Portal</h1>
          </div>
            ...
            {navigationItems.slice(0, 5).map((item) => {
              ...
              return (
                <button
                  key={item.id}
                  onClick={() => handleSectionChange(item.id)}
                  className={`flex flex-col items-center justify-center px-3 py-2 rounded-md transition-colors min-w-[60px] ${
                    activeSection === item.id
